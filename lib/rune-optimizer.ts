// @ts-nocheck The optimizer mirrors the exporter's variable JSON tuple format.
export type RunePlan = {
  version: number;
  generatedAt: string;
  account: string;
  mode: string;
  constraints: Record<string, boolean>;
  analyzedRuneCount: number;
  targetCount: number;
  recommendations: Array<{
    unitId: string; name: string; area: string; role: string;
    currentScore: number; suggestedScore: number; improvement: number;
    currentSets: string; suggestedSets: string;
    currentStats: { hpPct: number; atkPct: number; defPct: number; speed: number; critRate: number; critDamage: number; accuracy: number; resistance: number };
    suggestedStats: { hpPct: number; atkPct: number; defPct: number; speed: number; critRate: number; critDamage: number; accuracy: number; resistance: number };
    moves: Array<{ slot: number; runeId: string; set: string; level: number; replacesRuneId: string | null; from: string }>;
  }>;
  upgrades: Array<{ runeId: string; slot: number; set: string; level: number; rarity: number; main: { type: number; value: number; enchanted: boolean; grind: number }; substats: Array<{ type: number; value: number; enchanted: boolean; grind: number }>; suggestedRole: string; score: number }>;
};
...
export function optimizeRunes(raw: any, catalog: any): RunePlan {
